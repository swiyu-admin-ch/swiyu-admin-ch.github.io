...
toc: true
toc_sticky: true
excerpt: Detailed steps for the onboarding to the swiyu Sandbox Trust Infrastructure
header:
...
<div class="notice--danger">
  <h4 class="no_toc">Public Beta/Sandbox</h4>
...
Obtained from the [self-service portal](https://selfservice.api.admin.ch/api-selfservice/apis) during [step 3.3.](#33-save-your-tokens).
...
| `IDENTIFIER_REGISTRY_ENTRY_ID` | UUID of a specific DID space entry | The `id` field in the GET `/identifier/` response ([step 4.1.](#41-fetch-your-did-spaces)) |
| `IDENTIFIER_REGISTRY_URL` | Public URL where your DID log is hosted | The `identifierRegistryUrl` field in the GET `/identifier/` response ([step 4.1.](#41-fetch-your-did-spaces)) |

## 1.3. DID Toolbox

Generated by `didtoolbox create` in [step 4.2.](#42-generate-the-did-log). All key files are written to the `.didtoolbox/` directory in your working directory.
...
| Variable | Description | Where to get it |
|---|---|---|
| `VERIFIER_DID` | The DID of the verifier submitting the vqPS | Your registered DID from [step 4](#4-create-your-first-did) or [step 6](#6-adding-additional-dids) |
| `VQPS_SUBMISSION_ID` | UUID of the created vqPS submission | The `id` field in the POST `/vqps-submissions` response ([step 8.1.](#81-submit-the-vqps)) |
...
| DIDs hosted on the BR | max. 100 DIDs per BP allowed (Note for each DID costs will be charged) |
...
If you are registered with multiple business partners, click the business partner ID in the top-right corner of the self-service portal to select the one you want to subscribe with.
...
Use the [DID Toolbox](https://github.com/swiyu-admin-ch/didtoolbox-java/releases/latest) to generate a DID log for your reserved space. Pass the `identifierRegistryUrl` from step 4.1 as the `--identifier-registry-url` parameter.
...
Upload the generated `did.jsonl` to your reserved DID space using the `PUT` endpoint. Use the `IDENTIFIER_REGISTRY_ENTRY_ID` from [step 4.1](#41-fetch-your-did-spaces).
...
Upload the updated log to the Base Registry using the same `PUT` endpoint as in [step 4.3.](#43-upload-the-did-log):
